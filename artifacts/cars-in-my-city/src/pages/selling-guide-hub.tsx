...
import { sellingGuideTopics } from "./guide-data";
import { motion } from "framer-motion";
import { Sparkles, Camera, Tag, FileText, Handshake, ArrowRight, HelpCircle, BadgeDollarSign } from "lucide-react";
...
export default function SellingGuideHub() {
  const canonicalUrl = "https://carsinmycity.com/selling-guide";

  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: sellingGuideTopics.map((topic) => ({
      "@type": "Question",
      name: topic.question,
      acceptedAnswer: {
        "@type": "Answer",
  ...
  return (
    <Layout>
      <SEO
        title="The Complete Car Selling Guide"
        description="Everything you need to know before selling your car locally: prepping the car, taking great photos, pricing it right, writing a description, and completing a safe transaction."
        canonical={canonicalUrl}
        keywords={["how to sell a car", "car selling guide", "sell my car locally", "private car sale tips"]}
        jsonLd={faqSchema}
      />
      ...
      <section className="relative bg-primary pt-20 pb-24 md:pt-28 md:pb-32 overflow-hidden text-primary-foreground">
        ...
        <div className="container mx-auto px-4 relative z-10 max-w-5xl text-center">
          ...
          >
            The Complete Car <br className="hidden sm:block" />
            <span className="text-accent">Selling Guide</span>
          </motion.h1>
          ...
          >
            Five essential steps to selling your car locally with confidence — from prepping and
            photographing it right to closing a safe, fair deal.
          </motion.p>
      ...
      <div className="container mx-auto px-4 py-12 md:py-16 max-w-5xl">
        <div className="grid gap-5 sm:grid-cols-2">
          {sellingGuideTopics.map((topic) => {
            const Icon = icons[topic.icon];
            return (
              <Link key={topic.slug} href={`/selling-guide/${topic.slug}`}>
          ...
          <Card className="h-full p-6 border-dashed border-2 border-border flex flex-col items-center justify-center text-center bg-muted/30">
            ...
            <h2 className="text-base font-bold text-foreground mb-1">Buying instead?</h2>
